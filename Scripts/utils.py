...
def prev_follower_pitted_funct(lap_data, index):
...
def drivers_fastest_laps(lap_data):
    drivers_fastest = {}
    for un_d in lap_data["DriverNumber"].unique():
        fastest_lap_time = (
            lap_data.pick_drivers(un_d).pick_fastest()["LapTime"].total_seconds()
        )
...
def get_driver_num(lap_data, index):
    return lap_data.loc[index, "DriverNumber"]
...
def construct_lap_data(lap_data):
    ...
    lap_data[column_names] = np.nan

    fastest_laps = drivers_fastest_laps(lap_data)
    fastest_lap_of_race = lap_data.pick_fastest()["LapTime"].total_seconds()

    lap_data["FastestLap"] = np.nan
    lap_data["FastestLapOfRace"] = fastest_lap_of_race
    ...
    for index in lap_data.index:
        (
            ...
            prev_follower_stint,
            prev_follower_pace,
        ) = prev_follower_pitted_funct(index)
        (
            in_front_pace,
            in_front_time,
            in_front_stint,
            in_front_pitted,
        ) = get_in_front_info(index)
        pursuer_pace, pursuer_time, pursuer_stint = get_pursuer_info(index)
        ...
        for idx in range(len(var_names)):
            lap_data[column_names[idx]].iloc[idx] = var_names[idx]

        lap_data["FastestLap"].iloc[index] = fastest_laps[get_driver_num(index)]
    return lap_data
    ...
    for year in valid_years:
        ...
        for index, row in tqdm(schedule.iterrows()):
            if row["RoundNumber"] != 0:
                ...
                lap_data = construct_lap_data(lap_data)
                ...
                lap_data = lap_data[valid_lap_columns].reset_index(drop=True)
                lap_data.to_csv(lap_filepath, index=False)
